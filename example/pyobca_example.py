...
def parking():
    ...
    read_prototxt("./path_request.prototxt")
    ...
    # save_path=[]
    # for p in ds_path:
    #     save_path.append([p.x, p.y, p.heading])
    # np.savetxt("./path.txt",save_path)

    load_path = np.loadtxt("./path.txt")
    ds_path=[]
    for p in load_path:
        ds_path.append(pyobca.SE2State(p[0], p[1], p[2]))
