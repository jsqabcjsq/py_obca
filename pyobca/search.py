...
class SE2State:
    ...
    def get_next_states(self):
        #每次搜索6个方向，正前，正后，左前， 右前，左后，右后， 没有考虑车辆动力学
        #direction_index， 1，2，3是向前， 4，5，6是向后
...
def a_star_search(start: SE2State, goal: SE2State, grid_map, obstacles, search_cfg: SearchConfig = SearchConfig()):
    ...
    while((not q.empty()) and (it < max_it)):
        ...
        for next_state in state_current.get_next_states():
            ...
            max_heading_index = len(map[0][0])-1

            if next_state.x_index>max_x_index or next_state.y_index>max_y_index or next_state.heading_index>max_heading_index:
                continue 
